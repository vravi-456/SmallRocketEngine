...
legend_str = []
plt.figure(1)
T_throat = T_all[25,:,:] # all temperatures at throat station
for m in range(0, len(t_list)):
  plt.plot(d_list, T_throat[m,:])
  legend_str.append("t = %s seconds" % (t_list[m]));
plt.ylabel('Temperature [deg F]')
plt.xlabel('Distance [in]')
plt.title("Spatial temperature distribution at throat station")
plt.legend(labels=legend_str)
plt.axhline(y=T_melt)
plt.axvline(x=t_w_throat)
# plt.ylim([0, 200])
...
plt.ylabel('Temperature [deg F]')
plt.xlabel('Distance [in]')
plt.title("Spatial temperature distribution at chamber station")
plt.legend(labels=legend_str)
# plt.axhline(y=T_melt)
plt.axvline(x=t_w_barrelSection)

legend_str = []
plt.figure(3)
T_subsonic = T_all[23,:,:] # all temperatures in converging section
for m in range(0, len(t_list)):
  plt.plot(d_list, T_subsonic[m,:])
  legend_str.append("t = %s seconds" % (t_list[m]));
plt.ylabel('Temperature [deg F]')
plt.xlabel('Distance [in]')
plt.title("Spatial temperature distribution at random subsonic station")
plt.legend(labels=legend_str)
# plt.axhline(y=77)

# plt.figure(4)
# plt.plot(np.array(x_list) * m_to_in, np.array(r_list) * m_to_in, '.')
# plt.axis('equal')
# plt.axhline(y=0)